...
%              dW = sqrt(Delta).*randn(ninterv(j),1);
             ...
             for n = 1:ninterv(j)
                Y(:,n+1) = (eye(2) - A*Delta) \( (eye(2) + B*dW(n) + 1/2*[b^2 0; 0 b^2]*(dW(n)^2 - Delta)) * Y(:,n));
             end
             Yk(:,k) = Y(:,end);
             Xk(:,k) = X(:,end);
        end
        err(i) = mean(norm(Yk - Xk)); % I'm not sure what is the norm the book uses here
    end
...
% s = '90% confidence interval for mean approximation error using Implicite Milstein Approximation';
% sw = textwrap({s},60);
% title(sw);
...
s = 'plot of mean Implicite Milstein approximation error \epsilon against time step size |\Delta| in log2 scale';
sw = textwrap({s},60);
title(sw,'interpreter','LaTex');
set(gca, 'FontSize', 13)
saveas(h,'pcex12_2_4.jpeg')